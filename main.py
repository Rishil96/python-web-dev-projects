...
def search_cafe():
    ...
    search_result = db.session.execute(db.select(Cafe).where(Cafe.location == search_location))
    all_cafes = search_result.scalars().all()
    if all_cafes:
...
def update_coffee_price(cafe_id: int):
    ...
    update_cafe.coffee_price = updated_price
    db.session.commit()
    return jsonify(cafe=update_cafe.to_dict())
